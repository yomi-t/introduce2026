...
export default function SideDrawer({ isOpen, onClose }: Props) {
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* ドロワー本体 */}
          <motion.div
            className="absolute inset-y-0 left-0 w-3/4 bg-white z-30 flex flex-col shadow-2xl rounded-r-3xl overflow-hidden"
            initial={{ x: '-100%' }}
            animate={{ x: 0 }}
            exit={{ x: '-100%' }}
